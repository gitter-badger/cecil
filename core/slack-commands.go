...
func ListLeases(rawCtx interface{}) error {
	...
	for leaseIndex := range leases {
		...
		if lease.IsInstance() {
			raw, err := ctx.Slack.s.ResourceOf(&lease)
			if err != nil {
				return err
			}
		...
		if lease.IsStack() {
			leaseHeader := fmt.Sprintf("*STACK* on AWS %v:",
				lease.AWSAccountID,
			)
			...
			leaseHeader := fmt.Sprintf("*INSTANCE* on AWS %v:",
				lease.AWSAccountID,
			)
			response.WriteString(leaseHeader)
...
func ShowLease(rawCtx interface{}) error {
	...
	stackID, err0 := ctx.Params().GetString("stack-id")
	instanceID, err1 := ctx.Params().GetString("instance-id")
	leaseID, err2 := ctx.Params().GetInt("id")

	notEnoughInfo := err0 != nil && err1 != nil && err2 != nil
	if notEnoughInfo {
	...
	if instanceID != "" || stackID != "" {
		if instanceID != "" {
			lease, err = ctx.Slack.s.FetchLeaseByInstanceID(instanceID)
			if err != nil {
				if err == gorm.ErrRecordNotFound {
		...
		if stackID != "" {
			lease, err = ctx.Slack.s.FetchLeaseByStackID(stackID)
			if err != nil {
				if err == gorm.ErrRecordNotFound {
					return errors.New("lease not found")
				}
...
func TerminateLease(rawCtx interface{}) error {
	...
	ctx.Slack.s.TerminatorQueue.TaskQueue <- TerminatorTask{Lease: *lease}
...
func ExtendLease(rawCtx interface{}) error {
	...
	ctx.Slack.s.ExtenderQueue.TaskQueue <- ExtenderTask{
		Lease:     *lease,
		Approving: false,
	}
