...
// ShowCloudaccountOKLink runs the method Show of the given controller with the given parameters.
// It returns the response writer so it's possible to inspect the response headers and the media type struct written to the response.
// If ctx is nil then context.Background() is used.
// If service is nil then a default service is created.
func ShowCloudaccountOKLink(t *testing.T, ctx context.Context, service *goa.Service, ctrl app.CloudaccountController, accountID int, cloudAccountID int) (http.ResponseWriter, *app.CloudaccountLink) {
	// Setup service
	var (
		logBuf bytes.Buffer
		resp   interface{}
