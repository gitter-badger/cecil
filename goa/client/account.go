...
// --version=v1.0.0
//
// API "Cecil": account Resource Client
//
...
func (c *Client) RemoveMailerAccount(ctx context.Context, path string) (*http.Response, error) {
	req, err := c.NewRemoveMailerAccountRequest(ctx, path)
	if err != nil {
		return nil, err
	}
	return c.Client.Do(ctx, req)
...
// NewRemoveMailerAccountRequest create the request corresponding to the removeMailer action endpoint of the account resource.
func (c *Client) NewRemoveMailerAccountRequest(ctx context.Context, path string) (*http.Request, error) {
	scheme := c.Scheme
	if scheme == "" {
		scheme = "https"
	}
